<?php 
  ...
  if(isset($_POST) && $motty == 'load-pgd') {
    $n_tinhthanh  = isset($_POST['n_tinhthanh']) ? $_POST['n_tinhthanh'] : 0;
    $id_quanhuyen = isset($_POST['id_quanhuyen']) ? $_POST['id_quanhuyen'] : 0;

    $wh = "";
    if($n_tinhthanh != 0) {
      $wh .= " `num_3` = '$n_tinhthanh'";
    }
    if($id_quanhuyen != 0) {
      $wh .= $wh == "" ? " `num_4` = '$id_quanhuyen'" : " AND `num_4` = '$id_quanhuyen'"; 
    }
    $baiviet = LAY_baiviet(10, 0, $wh);
    echo '<option value="0">'.$glo_lang['chon_phong_giao_dich_gan_ban'].'</option>';
    if(count($baiviet)) {
  ...
  if(isset($_POST) && $motty == 'timkiem_pgd') {
    $n_tinhthanh  = isset($_POST['n_tinhthanh']) ? $_POST['n_tinhthanh'] : 0;
    $id_quanhuyen = isset($_POST['id_quanhuyen']) ? $_POST['id_quanhuyen'] : 0;
  ...
  if($motty == "send_form" && isset($_POST['send_lienhe']))
  {
    if((!empty($_POST['mabaove']) && strtolower($_POST['mabaove']) == strtolower($_SESSION['captcha']['code'])) || (!empty($_POST['id_token']) && $_POST['id_token']  == $_SESSION['token']) || (!empty($_POST['capcha_hd']) && $_POST['capcha_hd']  == $_SESSION['cap']))
    {
      ...
      $mutile_files        = "";
      $inputfile_mul_check = isset($_POST['inputfile_mul_check']) ? $_POST['inputfile_mul_check'] : 0;
      if(isset($_POST['inputfile_mul_check'])) {
        $upload_dir = "./datafiles/post/";  
        $allowed_types = array('jpg', 'svg', 'png', 'jpeg', 'gif' ,'ico');      
        foreach ($_FILES['mutile_files']['tmp_name'] as $key => $value) { 
            $file_tmpname = $_FILES['mutile_files']['tmp_name'][$key]; 
            $file_name    = time()."_".$_FILES['mutile_files']['name'][$key]; 
            $file_size    = $_FILES['mutile_files']['size'][$key]; 
            ...
            if(in_array(strtolower($file_ext), $allowed_types)) { 
                if(file_exists($filepath)) { 
                    ...
                    if( move_uploaded_file($file_tmpname, $filepath)) { 
                      if($inputfile_mul_check == 0) {
                        $mutile_files .= "~[".$file_name."]~|";
                      }
                      else {
                        $mutile_files .= "[[".$file_name."]]|";
                      }
      ...
      if($mutile_files != "") {
        $noidung       .= RETURN_text_lienhe(base64_encode("Đính kèm"), $mutile_files);
      }
      ...
      $data['mutile_files']        = LOC_char($mutile_files);
      $data['inputfile_mul_check'] = LOC_char($inputfile_mul_check);
  ...
  if($motty == "send_form" && isset($_POST['gui_donhang'])){
    ...
    if(isset($_SESSION['cart']) && $_SESSION['cart'] != NULL){
      ...
      $noidung       .= RETURN_text_lienhe($s_message_s, $s_message);
      $noidung       .= "</tr></table>";
      $noidung       .= '</tr><tr><td colspan="7" >'.$thongtin_dathang.'</td>';
  ...
  if($motty == 'update-qty'){
    ...
    if($qty == 0) {       
      if($keytn != "") {
        unset($_SESSION['cart'][$id."_".md5($keytn)]);
      }
      else {
