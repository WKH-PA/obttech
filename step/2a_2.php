    ...
    <div class="pro_view_more">
      <div class="leftBox" id="proView">
        ...
        <div class="viewRight">
          ...
          <div class="dv-tinhnang-dh">
            ...
            <div class="dv-ndd flex no_box">
              ...
              <label class="<?=$rows_2['icon'] != "" ? "img js_click_img" : '' ?>" <?=$rows_2['icon'] != '' ? 'onclick="$(\'a\',this).trigger(\'click\');' : '' ?>">
                <input type="radio" name="tinhnang_<?=$i ?>" class="js_tinhnang_getval" idname="<?=$arr_running['id']."_".$rows['id'] ?>" data="<?=$rows_2['id_val'] ?>" value="<?=$rows_2['id_val'] ?>" >
                <span class="<?=$rows_2['icon'] != "" ? "img" : '' ?>" style="<?=$rows_2['icon'] != '' ? 'background: url('.$fullpath."/".$rows_2['duongdantin']."/".$rows_2['icon'].');"' : '' ?>">
                  <?php if($rows_2['icon'] != ""){ ?>
                  <a href="<?=$fullpath."/".$rows_2['duongdantin']."/".$rows_2['icon'] ?>" class="cloud-zoom-gallery" rel="useZoom: 'zoom1', smallImage: '<?=$fullpath."/".$rows_2['duongdantin']."/".$rows_2['icon'] ?>'">
                  <?php } ?>
          ...
          <?php } ?>
        <div class="add_cart">
          <h3><a class="cur" onclick="$('#form_dathang').submit();"><?=$glo_lang['mua_ngay'] ?></a></h3>
          <ul>
            <li><a class="cur p_yeuthich p_yeuthich_<?=$arr_running['id'] ?>" onclick="yeu_thich(this, <?=$arr_running['id'] ?>)" ><i class="fa fa-heart-o"></i> <span><?=$glo_lang['yeu_thich'] ?></span> </a> </li>
            <li><a class="cur" onclick="popupwindow('https://www.facebook.com/sharer/sharer.php?kid_directed_site=0&sdk=joey&u=<?=$full_url."/".$motty ?>&display=popup&ref=plugin&src=share_button', 'title', 300, 300)"><i class="fa fa-share-alt"></i> <span><?=$glo_lang['chia_se'] ?></span> </a> </li>
            <?php
              $lienhe = LAY_step(6,1);
            ?>
            <li><a <?=full_href($lienhe) ?>><i class="fa fa-envelope-o"></i> <span><?=$glo_lang['lien_he'] ?></span> </a> </li>
            <div class="clr"></div>
          </ul>
        </div>
        </form>
