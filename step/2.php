<?php
  ...
  if(is_numeric($p) && $p > 0) {
    $lay_all_kx = LAYDANHSACH_idkietxuat($p, $slug_step);
    $wh .= "  AND (FIND_IN_SET('".$p."', `id_parent_muti`) OR (`id_parent` in (".$lay_all_kx."))) "; 
  }
...
<div class="right-new">
  <div class="sp-list-f pro_home_id flex <?=$is_search ? "js_timkiem_cls" : "" ?>">
    <?php
        if($nd_total == 0){
    ...
    ?>
      <ul>
        ...
        <!-- <div class="discount-tag" style="<?=$tinhnang_arr[$rows['num_3']]['ma_mau'] != "" ? "background: ".$tinhnang_arr[$rows['num_3']]['ma_mau'] : ""; ?>  "> -->
