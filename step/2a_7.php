...
<div class="pagewrap page_conten_page">
  <div class="right_sp">
    <div class="container"> 
        ...
        <div class="mySlides"> <img src="<?=checkImage($fullpath, $arr_running['icon'], $arr_running['duongdantin']) ?>" style="width:100%"> </div>
        <?php
        ...
        <div class="row">
          <div class="column"> <img class="demo cursor" src="<?=checkImage($fullpath, $arr_running['icon'], $arr_running['duongdantin']) ?>" style="width:100%" onclick="currentSlide(1)"> </div>
          <?php
            $i = 1;
            foreach ($list_hinhcon as $rows) {
              $i++;
          ?>
          <div class="column"> <img class="demo cursor" src="<?=checkImage($fullpath, $rows['icon'], $rows['duongdantin']) ?>" style="width:100%" onclick="currentSlide(<?=$i ?>)"> </div>
          <?php } ?>
        </div>
        <?php } ?>
      </div>
  ...
  <ul>
    ...
    <h4><a href="tel:<?=$thongtin['hotline_vi'] ?>"><i class="fa fa-volume-control-phone" ></i><?=$glo_lang['tu_van'] ?><span><?=$thongtin['hotline_vi'] ?></span></a></h4>
    </ul>
  </div>
  <div class="clr"></div>
  <div class="chitiet_sp ">
    ...
    <div class="chitiet_right chitiet_right_2">
      <div class="col-sm-3 col-xs-12 right-main">
        <div id="search-r">
          <input class="text input_search_enter_new" type="text" data=".input_search_enter_new" data-href="<?=$full_url ?>/search/?key=" placeholder="<?=$glo_lang['nhap_tu_khoa_tim_kiem'] ?>">
          <span id="searchSubmit"  onClick="SEARCH_timkiem('<?=$full_url ?>/search/?key=','.input_search_enter_new'); " style="cursor:pointer">
