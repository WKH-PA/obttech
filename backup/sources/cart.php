<?php
  ...
  if(isset($_POST['id'])){
    ...
    if($tinhnang != "") {
      $tinhnang_name = $id."_".md5($tinhnang);
      $_SESSION['tinhnang'][$tinhnang_name] = $tinhnang;
    }
    if($thongtin['is_giamuti'] == 1){
      if(isset($_POST['gia_muti'])) {
        ...
        if($tinhnang != "") {
          $tinhnang_name = $id."_".md5($tinhnang);
          $_SESSION['tinhnang'][$tinhnang_name] = $_POST['gia_muti'];
        }
      }
    }

    if(isset($_POST['qty_cart']) && is_numeric($_POST['qty_cart']) && $_POST['qty_cart'] > 0){
      $_SESSION['cart'][$tinhnang_name] = $_POST['qty_cart'];
    }
    else{
      $_SESSION['cart'][$tinhnang_name] = 1;
    }
...
<div class="link_page">
  ...
  <div class="pagewrap">
    ...
    <ul>
      ...
      <li><a href="<?=$full_url ?>"><i class="fa fa-home"></i><?=$glo_lang['trang_chu'] ?></a><span><i class="fa fa-angle-right"></i></span><a href="<?=$full_url."/gio-hang" ?>"><?=$glo_lang['gio_hang'] ?></a></li>
               ...
               <tr>
                  ...
                  <td  title="<?=$glo_lang['cart_qty'] ?>">
                    <div class="mobileqty no_box">
                      <input type='button' value='-' class='qtyminus' onclick="add_num_sp('#product-quantity-<?=$key ?>',-1); updateQty_notthis('<?=$full_url."/update-qty/" ?>', '<?=$id_sp ?>', '<?=@$_SESSION['tinhnang'][$key] ?>','<?=$key ?>');" />
                      <input type='text' min="1" max="9999" name='quantity' value='<?=$value ?>' class='qty qty_is_soluong' id="product-quantity-<?=$key ?>" onchange='updateQty("<?=$full_url."/update-qty/" ?>", "<?=$id_sp ?>","<?=$key ?>", this, "<?=@$_SESSION['tinhnang'][$key] ?>")' style="width: 50px" />
                      <input type='button' value='+' class='qtyplus' onclick="add_num_sp('#product-quantity-<?=$key ?>',+1); updateQty_notthis('<?=$full_url."/update-qty/" ?>', '<?=$id_sp ?>', '<?=@$_SESSION['tinhnang'][$key] ?>','<?=$key ?>');" />
                  </div>
                  ...
                  <td title="<?=$glo_lang['cart_thaotac'] ?>">
                    <form action="" method="post">
                      ...
                      <button type="submit" class="pro_del" name="xoa_sp" onclick="return confirm('<?=$glo_lang['ban_that_su_muon_xoa'] ?>')"><img src="images/cross.png" alt="delete" width="16px"></button>
                    </form>
                  </td>
                </tr>
              <?php  } } ?>
