<?php
    ...
    $lang       = "vi";
    $full_url   = $fullpath;
    if (in_array($motty, $lang_group) && $motty != "") {
        $lang       = $motty;
    ...
    if(isset($_GET['actilang'])) {
        $_SESSION["lang_acti"] = $motty;
    }
    if(!isset($_SESSION["lang_acti"]) && $thongtin['is_tiengviet'] == 0 && $lang != $lang_group[0]) {
        $_SESSION["lang"] = $lang_group[0];
        LOCATION_js(las_url_lang($lang_group[0], $motty, $haity, $baty, $bonty, $namty));
        exit();
    }
    // 
    $_SESSION["lang"]   = $lang;
    if($lang == "rs") $_SESSION["lang"]   = $lang = "cn";
    if($lang == "cn") $_SESSION["lang"]   = $lang = "in";
    //define
