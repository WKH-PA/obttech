...
if(isset($_GET['code'])) {
	try {
		...
		$check_fb = DB_que("SELECT * FROM `#_members` WHERE `email` = '".$email."' AND `phanquyen` = 0 LIMIT 1");
		if(!DB_num($check_fb)) {
		  $tentruycap = md5($user['id'].time());

		  DB_que("INSERT INTO `#_members`(`tentruycap`, `matkhau`, `hoten`, `email`, `id_google`,`google_icon`, `phanquyen`) VALUES('$tentruycap', '$tentruycap','".$hoten."','".$email."','".$id."', '$hinhanh','0')");

		  $check_fb = DB_que("SELECT * FROM `#_members` WHERE `email` = '".$email."' AND `phanquyen` = 0 LIMIT 1");
		  if(!DB_num($check_fb)){
		    ALERT_js("Địa chỉ email đăng ký đã tồn tại !");
		  ...
		  DB_que("UPDATE `#_members` SET `hoten` = '".$hoten."', `email` = '".$email."', `google_icon` = '$hinhanh' WHERE `email` = '".$email."' AND `phanquyen` = 0");
		  $check_fb = DB_arr($check_fb, 1);
