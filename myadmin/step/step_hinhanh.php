        ...
        <div class="dv-anh-chitiet-img-cont">
            <div class="dv-anh-chitiet-img">
                ...
                <input type="file" name="icon" id="input_icon" class="cls_hinhanh" accept="image/*"
                       onchange="pa_previewImg(event, '#img_icon','input_icon', 0);">
        ...
        <div class="dv-anh-chitiet-img-cont">
            <div class="dv-anh-chitiet-img">
                ...
                <input type="file" name="icon_hover" id="input_icon_hover" class="cls_hinhanh" accept="image/*"
                       onchange="pa_previewImg(event, '#img_icon_hover','input_icon_hover', 1);">
...
//                echo "(" . $size_img[0] * 1.5 . 'px x ' . $size_img[1] * 1.5 . "px)";
                echo "(" . $size_img[0] . 'px x ' . $size_img[1] . "px)";
            }
            ?></label>
        <div class="img-child">
            <div class=" no_box">
                <div class="js-conanh-step sortable-0 dv-anh-js-load dv-anh-js-load-0">
                    <?php
                    ...
                    foreach ($list_hinhcon as $r_img) {
                        ...
                        <div class="dv_hinhanh_con dv-anh-chl dv_hinhanh_con_<?= $r_img['id'] ?>">
                            <a class="cur jaax_sort_data jaax_sort_data-9" sort="<?= $r_img['sort'] ?>"
                               dataid="<?= $r_img['id'] ?>" onclick="remove_images_js(this, <?= $r_img['id'] ?>)"><img
                                        src="images/x_icon.svg" alt=""></a>
                            <img src="<?= full_src($r_img) ?>">
                        </div>
                    <?php } ?>
                </div>

                <div class="dv-dvjs-ajanh dv-dvjs-ajanh-0"></div>
                <div class="dv-anh-js-load-err dv-anh-js-load-err-0"></div>
                <div class="dv-add-anh dv-add-anh-0">
                    <input type="file" id="upload_mutile_tindang_0" multiple="multiple" style="padding: 0; height: auto"
                           accept="image/*">
                    +
                </div>
            </div>
...
<?php } ?>
<?php if (in_array($step, $danhmuc_360)) { ?>
    <div class="form-group">
        <label for="exampleInputFile2">Ảnh 360 <?php
            if (!empty($thongtin_step['size_img']) && $thongtin_step['size_img'] != '') {
                $size_img = explode("x", $thongtin_step['size_img']);
                echo "(" . $size_img[0] * 1.5 . 'px x ' . $size_img[1] * 1.5 . "px)";
            }
        ...
        <div class="img-child">
            <div class=" no_box">
                <div class="js-conanh-step sortable-1 dv-anh-js-load dv-anh-js-load-1">
                    <?php
                    $list_hinhcon = DB_fet("  * ", " `#_baiviet_img` ", " `id_parent` = '" . $id . "' AND `the_loai` = 1 ", " `sort` ASC, `id` ASC", "", "arr");
                    foreach ($list_hinhcon as $r_img) {
                        ?>
                        <div class="dv_hinhanh_con dv-anh-chl dv_hinhanh_con_<?= $r_img['id'] ?>">
                            <a class="cur jaax_sort_data jaax_sort_data-1" sort="<?= $r_img['sort'] ?>"
                               dataid="<?= $r_img['id'] ?>" onclick="remove_images_js(this, <?= $r_img['id'] ?>)"><img
                                        src="images/x_icon.svg" alt=""></a>
                ...
                <div class="dv-dvjs-ajanh dv-dvjs-ajanh-1"></div>
                <div class="dv-anh-js-load-err dv-anh-js-load-err-1"></div>
                <div class="dv-add-anh dv-add-anh-1">
                    <input type="file" id="upload_mutile_tindang_1" multiple="multiple" style="padding: 0; height: auto"
                           accept="image/*">
                    +
                </div>
            </div>
