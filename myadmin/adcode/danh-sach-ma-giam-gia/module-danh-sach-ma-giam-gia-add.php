  ...
  if(!empty($_POST))
    {
      ...
      $data['so_lan_su_dung']       = is_numeric(@$so_lan_su_dung) ? @$so_lan_su_dung : 0;
      $data['khong_gioi_han']       = is_numeric(@$khong_gioi_han) ? @$khong_gioi_han : 0;
      $data['loai_km']              = is_numeric(@$loai_km) ? @$loai_km : 0;
      $data['gia_tri_giam']         = is_numeric(@$gia_tri_giam) ? @$gia_tri_giam : 0;
      $data['ap_dung_cho']          = is_numeric(@$ap_dung_cho) ? @$ap_dung_cho : 0;
      // $data['gia_tri_ap_dung']      = !empty(${"gia_tri_ap_dung_".$ap_dung_cho}) ? ${"gia_tri_ap_dung_".$ap_dung_cho} : 0;

      $data['ap_dung_khuyen_mail_tren_don_hang'] = is_numeric(@$ap_dung_khuyen_mail_tren_don_hang)  ? @$ap_dung_khuyen_mail_tren_don_hang : 0;

      $data['bat_dau']              = is_numeric(@$bat_dau) ? @$bat_dau : 0;
      $data['ket_thuc']             = is_numeric(@$ket_thuc) ? @$ket_thuc : 0;
      $data['ngay_tao']             = is_numeric(@$ngay_tao) ? @$ngay_tao : 0;
