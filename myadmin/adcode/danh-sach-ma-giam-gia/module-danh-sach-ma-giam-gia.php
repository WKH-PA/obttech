<?php
  if(isset($_GET['them-moi']) || (isset($_GET['edit']) && is_numeric($_GET['edit']))){
    ...
    if(isset($_GET['del']))
    {
      $sql_se   = DB_que("SELECT * FROM `$table` WHERE `id`='".$_GET['del']."' LIMIT 1"); 
      ...
      if(DB_num($sql_se) > 0)
      {
        ...
        //xoa pr child
        DB_que("DELETE FROM `#_magiamgia_chitiet` WHERE `id_parent`  = '".$_GET['del']."' ");
        //
        $_SESSION['show_message_on'] = 'Xóa mã giảm giá thành công';
                        ...
                        <tbody>
                          ...
                          <tr>
                            ...
                            <td>
                              ...
                              <div>
                                <a style="font-size: 11px;" class="do" onclick="return confirm('Bạn thật sự muốn xóa?')" href="?module=quan-ly-thanh-toan&action=danh-sach-ma-giam-gia&del=<?=$ida ?>&token=<?=GET_token() ?>">[Xóa mã giảm giá]</a>
                              </div>
                            </td>
