  ...
  if($_SERVER['REQUEST_METHOD']=='POST')
    {
      ...
      foreach (LEFT_mainmenu_new() as $val) {
        ...
        // }
        // phan quyen danh muc
        if(in_array($val['id'], $array_only_bv)){
          $xem        = isset($_POST['dm_m_xem'.$val['id']]) ? 1 : 0;
          $them       = isset($_POST['dm_m_them'.$val['id']]) ? 1 : 0;
          $sua        = isset($_POST['dm_m_sua'.$val['id']]) ? 1 : 0;
          $xoa        = isset($_POST['dm_m_xoa'.$val['id']]) ? 1 : 0;
          $menu       = isset($_POST['dm_m_menu'.$val['id']]) ? 1 : 0;
          $list_phanquyen["dm_".$val['id']] = array("xem" => $xem,"them" => $them,"sua" => $sua,"xoa" => $xoa,"menu" => $menu);
        }
        // end
      }
    }
...
<form id="form_submit" name="form_submit" action="" method="post"  enctype='multipart/form-data'>
                ...
                <div class="form-group">
                  <table class="table table-hover table-danhsach table-danhsach-pq" style="margin: 0; width: 100%;">
                    ...
                    <?php
                      ...
                      <?php
                        if(in_array($val['id'], $array_only_bv)){
                          $xem  = !empty($list_phanquyen["dm_".$val['id']]['xem']) ? $list_phanquyen["dm_".$val['id']]['xem'] :0;
                          $them = !empty($list_phanquyen["dm_".$val['id']]['them']) ? $list_phanquyen["dm_".$val['id']]['them'] :0;
                          $sua  = !empty($list_phanquyen["dm_".$val['id']]['sua']) ? $list_phanquyen["dm_".$val['id']]['sua'] :0;
                          $xoa  = !empty($list_phanquyen["dm_".$val['id']]['xoa']) ? $list_phanquyen["dm_".$val['id']]['xoa'] :0;
                          $menu = !empty($list_phanquyen["dm_".$val['id']]['menu']) ? $list_phanquyen["dm_".$val['id']]['menu'] :0;
                      ?>
                      <tr dm_<?=$val['id'] ?>>
                        <td class="w80 text-center"><?=$cl ?></td>
                        <td class="dvhide-row">
                          <span class="sp-list-cap1">╚═►</span>
                          <div class="name name_list_cap_1" style="color: #FF5722;">
                            <?=$val['cataname'] ?> (danh mục)
                          </div>
                        </td>
                        <td class="w100 text-center">
                          <input type="checkbox" class='minimal' name="dm_m_xem<?=$val['id'] ?>" <?=(($xem == 1) ? "checked='checked'" : "" )?> >
                        </td>
                        <td class="w100 text-center">
                          <input type="checkbox" class='minimal' name="dm_m_them<?=$val['id'] ?>" <?=(($them == 1) ? "checked='checked'" : "" )?> >
                        </td>
                        <td class="w100 text-center">
                          <input type="checkbox" class='minimal' name="dm_m_sua<?=$val['id'] ?>" <?=(($sua == 1) ? "checked='checked'" : "" )?> >
                        </td>
                        <td class="w100 text-center">
                          <input type="checkbox" class='minimal' name="dm_m_xoa<?=$val['id'] ?>" <?=(($xoa == 1) ? "checked='checked'" : "" )?> >
                        </td>
                        <td class="w100 text-center">
                          <input type="checkbox" class='minimal' name="dm_m_menu<?=$val['id'] ?>" <?=(($menu == 1) ? "checked='checked'" : "" )?> >
                        </td>
                      </tr>
                      <?php } ?>
                      <!-- danh muc end-->
  ...
  <h3 class="box-title box-title-td pull-right">
    <button type="submit" class="btn btn-primary"><i class="fa fa-floppy-o"></i> <?=glo_lang_admin('Lưu lại') ?></button>
    <a href="<?=$url_page ?>&them-moi=true" class="btn btn-primary"><i class="fa fa-plus"></i> Thêm mới</a>
    <a href="<?=$url_page ?>" class="btn btn-primary"><i class="fa fa-sign-out"></i> Thoát</a>
  </h3>
</div>
