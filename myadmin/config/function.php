<?php 
	...
	function SHOW_text($text, $glo_get_qc = ''){	
	    ...
	    if(defined("MOTTY")) {
	    	...
	    	$text = preg_replace('/<a(.*?)href="(.*?)\.jpg"(.*?)>/i', '<a$1href="$2.jpg"$3 class="lightbox">', $text);
	    	$text = preg_replace('/<a(.*?)href="(.*?)\.jpeg"(.*?)>/i', '<a$1href="$2.jpeg"$3 class="lightbox">', $text);
	    	$text = preg_replace('/<a(.*?)href="(.*?)\.png"(.*?)>/i', '<a$1href="$2.png"$3 class="lightbox">', $text);
	    	$text = preg_replace('/<a(.*?)href="(.*?)\.gif"(.*?)>/i', '<a$1href="$2.gif"$3 class="lightbox">', $text);
	    	
	    	
	    	$text = str_replace('http://khamtimmach.com/datafiles/', '/2020_khamtimmach/datafiles/images/data/', $text);
	    }
	    else {
	...
	function LAY_khuvuc($limit = 0, $where = ""){
		if($limit == 0)  $limit = "";
		if($where != "") $where = " AND $where";
		$danhmuc  = DB_fet_rd("*","`#_ship_khuvuc`","`showhi` = 1 $where ","`catasort` ASC, `id` DESC",$limit, "id");
		return $danhmuc;
	}
	...
	function full_src($rows, $thumb = 'thumb_')
		{
			global $fullpath;
			$check_time = "";
			if(!empty($rows['time_dongdau'])) {
				$check_time = "?v=".$rows['time_dongdau'];
			}
			if($rows['icon'] != '') {
				return $fullpath."/".$rows['duongdantin']."/".$thumb.$rows['icon'].$check_time;
			}
			else {
				return $fullpath."/myadmin/images/no_image_400_312.jpg";
			}
	...
	function full_img($rows, $thumb = 'thumb_'){
		...
		return '<img  src="" class="isload isload_full isload_full_1" data-original="'.$link_img.$check_time.'" alt="'.$rows['tenbaiviet_'.$lang].'" />';
	}
    ...
    function get_tinnang_input($idtn, $idbv, $lang = "vi", $loaihienthi = 0, $tinhnang = array()) {
    	if($loaihienthi == 0) {
	    	$check = DB_que("SELECT `tenbaiviet_".$lang."` FROM `#_baiviet_select_tinhnang` WHERE `id_baiviet` = '$idbv' AND `id_tinhnang` = '".$idtn."' AND `loaihienthi` = 0 LIMIT 1");
	    	if(!DB_num($check)) {
	    		return "";
	    	}
	    	$check = DB_arr($check, 1);
	    	return $check['tenbaiviet_'.$lang];
    	}
    	else {
    		$check = DB_que("SELECT `id_val` FROM `#_baiviet_select_tinhnang` WHERE `id_baiviet` = '$idbv' AND `id_tinhnang` = '".$idtn."' AND `loaihienthi` = 1 LIMIT 1");
	    	if(!DB_num($check)) {
	    		return "";
	    	}
	    	$check = DB_arr($check, 1);
	    	$check = $check['id_val'];
	    	return $tinhnang[$check]['tenbaiviet_'.$lang];
    	}
    }
    function DV_ipv4_ipv6($ip) {
    	if(strlen($ip) > 32){
		    return hexdec(substr($ip, 0, 2)). "." . hexdec(substr($ip, 2, 2)). "." . hexdec(substr($ip, 5, 2)). "." . hexdec(substr($ip, 7, 2));
		}
		return $ip;
    }
    function watermark_image($file, $destination, $overlay = "images/watermark.png", $X = 0, $Y = 0){
	...
	function dongdau_anh($icon_bv_thmb, $table = '#_baiviet', $id = 0){
		global $thongtin;
		$dongdau_img = $thongtin['dongdau_img'];
		$dongdau_img = "../".$thongtin['duongdantin']."/".$dongdau_img;
		if($dongdau_img == "") {
			exit();
		}

		@watermark_image($icon_bv_thmb, $icon_bv_thmb, $dongdau_img);
		DB_que("UPDATE $table SET `is_dongdau` = 1, `time_dongdau` = '".time()."' WHERE `id` = '$id' LIMIT 1");
	}
?>