<?php
  ...
  define("_lang_nb3", "Rusia");
  define("_lang_nb3_key", "cn");
  
  define("_lang_nb4", "Japan");
  define("_lang_nb4_key", "jp");

  define("_lang_nb5", "Korea");
  define("_lang_nb5_key", "kr");

  define("_lang_nb6", "China");
  define("_lang_nb6_key", "in");
  ...
  $arr_list_lang = array();
  $arr_list_lang['vi'] = "Vi";
  if($lang_nb2) $arr_list_lang['en'] = "En";
  if($lang_nb3) $arr_list_lang['cn'] = "Ru";
  if($lang_nb4) $arr_list_lang['jp'] = "Jp";
  if($lang_nb5) $arr_list_lang['kr'] = "Kr";
  if($lang_nb6) $arr_list_lang['in'] = "Cn";

// zh-CN
  $array_dich = array("en" => "en", "cn" => "ru", "jp" => "ja", "kr" => "ko", "in" => "zh-CN");
  ...
  foreach ($module_setting as $rows) {
    ...
    if($rows['id'] == 73) $danhmuc_360  = explode(",", $rows['ten_key']);
    
    if($rows['id'] == 74) $module_tenhienthi  = explode(",", $rows['ten_key']);
    if($rows['id'] == 22) $module_mota  = explode(",", $rows['ten_key']);
    if($rows['id'] == 75) $module_google  = explode(",", $rows['ten_key']);
    if($rows['id'] == 76) $module_hinhanh  = $rows['ten_key'];
